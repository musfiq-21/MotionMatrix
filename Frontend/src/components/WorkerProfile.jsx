...
export default function WorkerProfile({ user, currentUser }) {
  ...
  useEffect(() => {
    const fetchFloorManager = async () => {
      try {
        const token = localStorage.getItem('authToken');
        
        if (!user?.assignedFloorId) {
          console.log('⚠️ Worker has no assigned floor');
          return;
        }

        // Get floor manager for the worker's floor
        const response = await fetch(`http://localhost:5000/api/users/floor-manager/${user.assignedFloorId}`, {
          headers: {
        ...
        if (response.ok) {
          ...
          if (data.data) {
            setFloorManager(data.data);
          }
        }
      } catch (error) {
        console.error('Error fetching floor manager:', error);
      }
    };
    ...
    if (user?.id && user?.assignedFloorId) {
      fetchFloorManager();
    }
  }, [user]);
      ...
      <div className="work-info-section">
        ...
        <div className="info-grid">
          ...
          <div className="info-box">
            <span className="info-icon">👨‍💼</span>
            <div className="info-content">
              <p className="info-label">Floor Manager</p>
              <p className="info-value">{floorManager?.name || 'Not assigned'}</p>
            </div>
          </div>
